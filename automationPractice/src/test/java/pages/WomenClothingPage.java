...
public class WomenClothingPage {
    ...
    public ProductPage openRandomClothingPage() throws InterruptedException {
        int productTotal = getProductCountFromHeader();

        if(productTotal == 1) {
            WebElement product = driver.findElement(By.cssSelector("#center_column > ul > li.ajax_block_product.col-xs-12.col-sm-6.col-md-4.first-in-line.first-item-of-tablet-line.first-item-of-mobile-line > div > div.right-block > h5 > a"));
            ...
            WebElement product = productList.get(listItem-1).findElement(By.cssSelector("div > div.right-block > h5 > a"));
            waitPage.until(ExpectedConditions.elementToBeClickable(product));
